...
// --- UPDATED SELECTOR ---
// This selector is more robust for finding both stream and category cards.
function getDirectoryItemNodes(mode) {
    if (!mainContentNode) return [];
    const suffix = mode === 'unprocessed' ? ':not([data-uttv-processed])' : '';
    const selector = `div.tw-tower > div${suffix}`; // Target direct children of the main content grid
...
function getSidebarItemNodes(mode) {
    ...
    const suffix = mode === 'unprocessed' ? ':not([data-uttv-processed])' : '';
    // This targets the container of a followed channel in the sidebar
    const selector = `.side-nav-card${suffix}`;
...
function readChannel(containerNode) {
    // Find key elements within the card
    const linkNode = containerNode.querySelector('a[data-a-target="preview-card-image-link"]');
    if (!linkNode) return null;

    const titleNode = containerNode.querySelector('a[data-a-target="preview-card-title-link"] > h3');
    const nameNode = containerNode.querySelector('a[data-a-target="tw-card-avatar-link"]');
    const categoryNode = containerNode.querySelector('a[data-a-target="preview-card-game-link"]');
    const tagContainer = containerNode.querySelector('div[data-a-target="preview-card-tags"]');

    const name = nameNode?.getAttribute('href')?.substring(1) ?? ''; // Get name from the user's link href
    if (!name) return null;
    ...
    return {
        ...
        rerun: !!containerNode.querySelector('[aria-label*="Rerun"], [data-a-target="stream-rerun-indicator"]'),
