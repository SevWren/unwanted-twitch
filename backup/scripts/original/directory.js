...
//logVerbose('Checking blacklist for item:', item.name || item.category || item.title, 'Extracted Tags:', item.tags?.map(t => t.name)); // Verbose Log extracted tags
//line 1084 changed to logVerbose('Extracted tags from container:', tags.map(t => t.name), tagContainerNode);
